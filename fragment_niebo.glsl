...
void main(void) {
    ...
    float nl[8];
    float rv[8];
    vec4 kd=mix(texture(textureMap0,iTexCoord0),texture(textureMap1,iTexCoord1),0.02);  //Kolor materialu dla swiatla  rozpraszanego
    vec4 ld=vec4(0.4,0.4,0.4,1); 
    vec4 ks=vec4(0.5,0.5,0.5,1); 
    vec4 ls=vec4(0.05,0.05,0.05,1); 
    ...
    for (int i=0;i<8;i++) {
        ...
        nl[i]=clamp(dot(mn,ml[i]),0,1); //cos kata pomiedzy wektorami n i l
